...
{
    uint64_t l2_idx = calc_index_l2(VIRTIO_REG_RANGE);
    uint64_t l2_desc2 = make_block_desc(reg); // that we got from the DTD parsing
...
void mmu_init(void)
{
    ...
    // for DTD 0x8xx -> 0x40000000
    l2_idx = calc_index_l2(VIRT_DTB_BASE);
    l2_desc2 = make_block_desc(DTB_BASE);
    set_table_entry((uint64_t*)L2_BASE_PHYS, l2_idx, l2_desc2);
